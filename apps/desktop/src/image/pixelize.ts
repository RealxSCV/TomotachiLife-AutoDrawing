...
export async function pixelizeImage(
  ...
  let pixelMap = enableDenoise ? removeIsolatedPixels(collapsedPixelMap, profile.colorCount) : collapsedPixelMap;
  
  // 限制颜色数量（对非mono模式）
  if (profile.colorMode !== "mono") {
    pixelMap = limitPixelMapColors(pixelMap, profile.colorCount);
  }
