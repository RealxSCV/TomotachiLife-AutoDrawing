...
function syncStudioUi() {
  ...
  const brushShapeHint =
    normalizeBrushShapeValue(state.studio.brushShape) === "square"
      ? `当前预设是 ${selectedBrushPresetLabel}；开始绘制时设备会先按 X、X 进入笔刷页，再从默认的 7 像素圆点笔刷自动切到这个方块像素笔刷，并连按两次 A 完成选中和返回画布；回到画布后还会额外等待约 3 秒再继续。进入绘画页后不要再手动改笔刷，也不要再移动页面。`
      : state.studio.brushSize === 1
        ? "当前预设是 1 像素圆形像素笔刷；开始绘制时设备会自动打开笔刷页，切换到这一档后再连按两次 A 返回画布，并等待约 3 秒再继续。"
